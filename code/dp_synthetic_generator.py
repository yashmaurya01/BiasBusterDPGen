...
Note: Do not mention anything else, simply begin with the new sample.
	...
	def generate_next_logprob(self, samples, generated_till_now=None):
		...
		exp_mech = Exponential(epsilon=self.epsilon, sensitivity=1.0, utility=[o['p'] for o in options])
		index_of_choice = exp_mech.randomise()
		return options[index_of_choice]['message'], options[index_of_choice]['finish_reason']
	...
	def generate_new_point(self, self_max_tokens=100):
		...
		for i in trange(self_max_tokens):
			if len(new_string)==0:
				next_out, stop_reason = self.generate_next_logprob(batch_txt)
			else:
				next_out, stop_reason = self.generate_next_logprob(batch_txt, new_string)
			new_string += next_out
