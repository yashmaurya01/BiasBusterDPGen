	...
	def generate_next_logprob(self, samples, generated_till_now=None, quote_in=False):
		...
		if generated_till_now is None:
			...
			messages = [
				{"role": "system", "content": self.PROMPT_TEMPLATE},
				{"role": "user", "content": "Here are some samples from the private dataset:\n\n\"\"\"\n"+samples+"\n\"\"\"\n\nSynethetic Sample:\n"},
				{"role": "system", "content": "Remember the samples provided, follow the structure given. No double quotes or incorrect ',' placements. Do not repeat '\"', you should never generate a '\"\"' or a '\"\"\"'. Ensure that you will continue from where you left off in your response after your last line as if nothing happened, ensuring you will not repeat anything, here is the rest of the response you've generated till now! Do not repeat \" or , if you previously mentioned them. Remember whatever you output will be directly appended to the previous string, so if the previous string ends with \" do not begin your next text with it. Understand that you're continuing the previous string as it is. You are not creating your own stuff, use as few \" as you can."},
